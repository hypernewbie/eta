...
type Peer = {
  ...
  accent: string;
  glyph: string;
  // Set only for a PC reached by running Eta on it over SSH. It is that
  // PC's durable identity: url is a forwarded port that differs every
...
type Entry = {
  ...
  size: number;
  modified: string;
  // Set by the server: a dotfile anywhere, or a file carrying the
...
function showToast(message, variant = "danger") {
  const alert = document.querySelector("#error-toast") as
    | (HTMLElement & { variant?: string; toast?: () => void })
    | null;
  if (!alert) {
  ...
  (alert.querySelector("#error-message") as HTMLElement).textContent = message;
  alert.toast();
}
...
function setShowHidden(show: boolean) {
...
function syncHiddenToggle(view: ExplorerView) {
...
function renderStatusBar(view: ExplorerView, entries: Entry[], hidden = 0) {
...
function markdownToSafeHTML(raw: string): string | null {
...
function openShortcut(shortcut: DesktopShortcut, peer: Peer | null) {
...
type DesktopIcon = {
  ...
  label: string;
  title: string;
  // The machine this icon belongs to. Drives the accent, so a peer's
  // folder is coloured by the peer that holds it, not by this host.
  peer: Peer | null;
  art: DesktopIconArt;
  open: () => void;
  removable: boolean;
...
function desktopIconModel(roots: Root[]): DesktopIcon[] {
...
function desktopIconMarkup(icon: DesktopIcon) {
  const art =
    "glyph" in icon.art
...
async function refreshPeerIdentities() {
  ...
  if (
    peerIdentityFingerprint(latest) === peerIdentityFingerprint(enrolledPeers)
  ) {
    return;
  }
  ...
  for (const view of explorerViews.values()) {
    ...
    const updated = enrolledPeers.find(
      (peer) => peer.url === view.state.peer!.url,
    );
    if (updated) view.state.peer = updated;
  }
...
function volumeIcon() {
  if (mediaMuted || mediaVolume === 0) return "volume-x";
  if (mediaVolume < 0.34) return "volume";
  if (mediaVolume < 0.67) return "volume-1";
  return "volume-2";
}
function applyMediaVolume(scope: ParentNode = document) {
...
function syncVolumeUI() {
  ...
  ($("#volume-slider") as HTMLInputElement).value = String(percent);
  $("#volume-value").textContent = mediaMuted ? "off" : String(percent);
  iconify();
}
function setVolume(value: number, muted = mediaMuted) {
...
function initTray() {
  ...
  $("#volume-slider").addEventListener("input", (event) => {
    setVolume(Number((event.target as HTMLInputElement).value) / 100, false);
  });
  // Scrolling over the tray icon adjusts volume, as it does on a
  // desktop taskbar.
  $("#volume-button").addEventListener(
    "wheel",
    (event) => {
      event.preventDefault();
  ...
  document.addEventListener("pointerdown", (event) => {
    if (
      !(event.target as HTMLElement).closest("#volume-popover, #volume-button")
    ) {
...
async function updateDesktopWidgets() {
  ...
  if (now.getSeconds() % 2 === 0) {
    try {
      ...
      let totalClusterBytes =
        (localStats?.txBytes || 0) + (localStats?.rxBytes || 0);

      const onlinePeers = enrolledPeers.filter((p) => isPeerOnline(p));
      for (const peer of onlinePeers) {
        const peerKey = peer.name || peer.url;
        const peerStats = await api(
          `/api/remote/stats/network?peer=${encodeURIComponent(peerKey)}`,
        ).catch(() => null);
        if (peerStats) {
      ...
      if (txEl) txEl.textContent = `${totalTxSpeed.toFixed(1)} KB/s`;
      const rxEl = $("#widget-net-rx-speed");
      if (rxEl) rxEl.textContent = `${totalRxSpeed.toFixed(1)} KB/s`;

      const totalEl = $("#widget-net-total-bytes");
      if (totalEl) totalEl.textContent = `Total: ${bytes(totalClusterBytes)}`;
...
$("#desktop-icons").addEventListener("contextmenu", (event) => {
  ...
  $("#desktop-context-menu [data-desktop-action='peer-password']").hidden =
    !model.peer;
  $("#desktop-context-menu [data-desktop-action='remove-peer']").hidden = !(
    isComputerIcon && model.peer
  );
...
$("#desktop-context-menu").addEventListener("click", async (event) => {
  ...
  if (action.dataset.desktopAction === "reconnect-pc") {
    ...
    if (destination) await reconnectRemotePC(destination);
    return;
  }
  if (action.dataset.desktopAction === "disconnect-pc") {
    const peer = desktopIconIndex.get(key)?.peer;
    if (!peer?.ssh_destination) return;
    try {
      await api(
  ...
  if (action.dataset.desktopAction === "cleanup-pc") {
    ...
    if (
      !window.confirm(
        `Delete Eta's files from ${peer.ssh_destination} (~/.eta) and forget this PC?\n\nNothing else on that computer is touched.`,
      )
    )
      return;
    try {
...
async function pastePeerList() {
  ...
  const targets = (urls as string[]).filter((url) => url && !known.has(url));
  if (targets.length === 0) {
  ...
  for (const url of targets) {
    try {
      const peer = await addPeer(url).catch(async (error) => {
        if (
          !(error as { peerPasswordRequired?: boolean }).peerPasswordRequired
        ) {
          throw error;
        }
  ...
  const skipped = (urls as string[]).length - targets.length;
  if (skipped) parts.push(`${skipped} already added`);
  showToast(
    parts.length ? `Pasted PC list: ${parts.join(", ")}` : "Nothing to add",
    failed && !added ? "danger" : "success",
  );
}
$("#eta-menu-copy-peers").addEventListener("click", () => void copyPeerList());
$("#eta-menu-paste-peers").addEventListener(
  "click",
  () => void pastePeerList(),
);
...
async function removePeer(peer: Peer) {
  try {
    const response = await fetch(
      `/api/peers?url=${encodeURIComponent(peer.url)}`,
      { method: "DELETE" },
    );
...
const SETUP_PHASE_TEXT: Record<string, string> = {
  connecting: "Connecting over SSH…",
  checking: "Checking what's there…",
  installing:
    "Installing Eta (first time on a PC builds it, so this can take a few minutes)…",
...
function refreshReconnectAllButtonVisibility() {
  const btn = $("#reconnect-all-button") as HTMLElement | null;
  if (btn) btn.hidden = !enrolledPeers.some((p) => Boolean(p.ssh_destination));
}

function reconnectAllRemotePCs() {
  const sshPeers = enrolledPeers.filter((peer) =>
    Boolean(peer.ssh_destination),
  );
...
$("#settings-changelog-button").addEventListener("click", async () => {
  ...
  try {
    ...
    content.innerHTML = `<p class="preview-note">${escapeHTML((error as Error).message)}</p>`;
  }
});
$("#changelog-close").addEventListener("click", () =>
  $("#changelog-dialog").hide(),
);
