...
// add the common locations before the command -v check, and to test for
// the binary at each rather than hoping PATH resolves it.
func TestRemoteCommandPutsGoOnPath(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(posix, want) {
			t.Errorf("posix install script missing %q; full script:\n%s", want, posix)
		}
	}
}
